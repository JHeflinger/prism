...
static void DrawEditPanel(float width, float height) {
    if (g_item_selected) {
        if (g_edit_type == EDIT_MATERIAL) {
            ...
            UIMoveCursor(0, 15);
            UIMoveCursor((width / 2) - (UITextWidth("Scattering") / 2) - 10, 0);
            UIDrawText("Scattering");
            UIMoveCursor(0, 5);
            UIDrawText("r");
            UIMoveCursor(15, -20);
            edited |= UIDragFloat(&(matref->scattering[0]), 0, FLT_MAX, 0.05f, component_width);
            UIMoveCursor(component_width + 25, -20);
            UIDrawText("g");
            UIMoveCursor(component_width + 40, -20);
            edited |= UIDragFloat(&(matref->scattering[1]), 0, FLT_MAX, 0.05f, component_width);
            UIMoveCursor((2*component_width) + 50, -20);
            UIDrawText("b");
            UIMoveCursor((2*component_width) + 65, -20);
            edited |= UIDragFloat(&(matref->scattering[2]), 0, FLT_MAX, 0.05f, component_width); 
            UIMoveCursor(0, 35);
